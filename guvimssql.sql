...
select District_code, LPG_or_PNG_Households from census_2011 GROUP BY District_code;


--What is the religious composition (Hindus, Muslims, Christians, etc.) of each district?

select district_code,sum(isnull(Hindus, 0) + isnull(Muslims, 0)+isnull(Christians, 0)+isnull(Sikhs, 0)
+isnull(Buddhists, 0)
...
select District_code, sum(Households_with_Internet) from census_2011 group by District_code
...
select sum (x.HouseHoldPerDistrict) totalHouseHolds, [State/UT] from
(select [State/UT]
...
select sum(Having_latrine_facility_within_the_premises_Total_Households) HouseHolds, [State/UT]
from census_2011 group by [State/UT]

--What is the average household size in each state?
select avg (x.HouseHoldPerDistrict) AverageHouseHoldSize, [State/UT] from
(select [State/UT]
,isnull(Household_size_1_person_Households,0)
+isnull(Household_size_2_persons_Households,0)
+isnull(Household_size_1_to_2_persons,0)
...
select [State/UT], sum(Ownership_Owned_Households) OwnedHHold,
...
select [State/UT], sum(Location_of_drinking_water_source_Near_the_premises_Households) WaterNearPremises
 from census_2011
group by [State/UT]
...
select [State/UT], sum(ltPercent) OverallLtR from (
